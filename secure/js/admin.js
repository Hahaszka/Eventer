$(document).ready(function () {
  ...
  function loadUsers() {
    ...
    $.ajax({
        ...
        response.forEach((u) => {
          ...
          const badges = `
                        ...
                        ${
                          u.is_active
                            ...
                            : '<span class="badge bg-secondary">ZBANOWANY</span>'
                        }
                        ${
                          u.is_verified
                            ? '<span class="badge text-bg-light border">WERYF.</span>'
                            : ""
                        }
                    `;

          const safeUserJson = JSON.stringify(u)
            .replace(/'/g, "&apos;")
            .replace(/"/g, "&quot;");
          ...
          const row = `
                        <tr>
                            ...
                            <td>${escapeHtml(firstName)} ${escapeHtml(
            lastName
          )}</td>
      ...
      error: function (xhr) {
        console.error("--> Błąd API:", xhr.status);

        if (xhr.status === 403) {
          alert("Brak uprawnień administratora. Przekierowanie do Dashboardu.");
