$(document).ready(function () {
  ...
  function initProfile() {
    $.ajax({
      ...
      success: function (me) {
        ...
        if (isMyProfile) renderProfileData(me, true);
        else fetchPublicProfile(targetId);
      },
      error: function (xhr) {
  ...
  function loadUserEvents(userId, isOwner) {
    ...
    $.ajax({
      ...
      success: function (events) {
        ...
        if (events.length === 0) {
          $container.html(
            `<div class="text-center py-5 text-muted"><p>${
              isOwner
        ...
        events.forEach((event) => {
          ...
          const categoryBadge = Utils.getCategoryBadge(event.category);
          ...
          if (isOwner) {
            ...
            buttonsHtml = `
              <div class="d-flex justify-content-end gap-2 mt-3">
                  <button class="btn btn-sm btn-outline-dark edit-event-btn" data-event="${eventData}"><i class="fas fa-edit me-1"></i> Edytuj</button>
                  <button class="btn btn-sm btn-outline-danger delete-event-btn" data-id="${event.id}"><i class="fas fa-trash me-1"></i> Usuń</button>
              </div>`;
          }
          ...
          $container.append(`
              <div class="card border-0 shadow-sm rounded-4 mb-4 overflow-hidden">
                  <div style="height: 180px; background-image: url('${imgUrl}'); background-size: cover; background-position: center;">
                      <div class="w-100 h-100 d-flex align-items-end justify-content-between p-3" style="background: linear-gradient(to top, rgba(0,0,0,0.6), transparent);">
                          <span class="badge bg-white text-dark rounded-pill shadow-sm"><i class="fas fa-calendar-alt me-1"></i> ${dateStr}</span>
                          ${categoryBadge}
                      </div>
                  </div>
                  <div class="card-body p-4">
                      <h5 class="fw-bold mb-1 text-dark">${Utils.escapeHtml(event.title)}</h5>
                      <p class="text-muted small mb-0">${Utils.escapeHtml(event.description || "Brak opisu.")}</p>
                      ${buttonsHtml}
                  </div>
              </div>
          `);
  ...
  $("#editProfileForm").submit(function (e) {
    ...
    $.ajax({
      ...
      error: function (xhr) {
        $("#profileAlert")
          .text("Błąd: " + Utils.getApiError(xhr))
          .removeClass("d-none");
      },
    });
  });
  ...
  $(document).on("click", ".edit-event-btn", function () {
    ...
    $("#editEventCategoryInput").val(eventData.category || "OTHER"); 
    $("#editEventDateInput").val(Utils.toLocalIsoString(eventData.event_date));
  ...
  $(document).on("click", ".delete-event-btn", function () {
    ...
    $.ajax({
      ...
      error: function(xhr) {
          alert("Nie udało się usunąć: " + Utils.getApiError(xhr));
      }
    });
  });
